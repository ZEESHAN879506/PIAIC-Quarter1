...
class Student {
    ...
    async addBalance() {
        ...
        if (validateNum(answer.amount) && answer.amount > 0) {
            ...
            console.log(chalk.bgRed(`\t Some Thing Went Wrong `));
    }
    async viewBalance() {
        console.log("\n");
    ...
    async addCourse() {
        ...
        this.enrollCourses.map(y => {
            if (y.courseName === answer.courses)
                found = true;
        });
        if (!found) {
            ...
            if (this.balance > newcourse.courseFee) {
                ...
                console.log(chalk.bgRed(`\t Your Account Balance: ${this.balance} is Less Then Course Fee : ${newcourse.courseFee} Please Add Balance First `));
        }
        else {
    ...
    async viewCourses() {
        if (this.enrollCourses.length > 0) {
            ...
            console.log(chalk.bold.bgRed.inverse("\t You Are Not Enrolled In Any Courses Yet: "));
    }
}
const newStudent = new Student("123", "Muhammad Zeeshan");
let Students = [newStudent];
async function enableFunctionality(loginStudent) {
    let repeat = true;
    while (repeat) {
        const answer = await inquirer.prompt([
            {
...
async function signUp() {
    ...
    let newstudent = new Student(answer.password, answer.name);
    console.log("\n");
