...
function sleep(time:number){
    ...
    const myList:string[]=[];
     ...
     function myTasks(){
        ...
        else console.log(chalk.italic.bgRed("TODO List is Empty "));
    }
        ...
        while(repeat) {
           const tasks= await inquirer.prompt({
            ...
            { if(answer.priority){
              ...
              else myList.push(tasks.task)
            }
        else { console.log(chalk.italic.bgRed(`Task cannot be Empty`)); answer.repeat=true;}
            if(!answer.repeat) repeat=false;
        } 
    }



    function deleteTaskNo(num:number){  
        myList.splice(num,1) ;
    ...
    async function deleteTask(){
        ...
        if(myList.length != 0){
            ...
            while(repeat) {
                    ...
                    if( myList.length >= taskNo.num)  await deleteTaskNo(taskNo.num);        
                    else{ console.log(chalk.italic.bgRed("Please Enter correct Task No (Example  =>   1)"));  await deleteTask(); return;}
                    ...
                    else {console.log(chalk.italic.bgRed("TODO List is Empty ")); return;}
            }
        }
        else console.log(chalk.italic.bgRed("TODO List is Empty ")); 
