    ...
    while (repeat) {
        ...
        if (validateNum(User.Id) && validateNum(User.Password)) {
            ...
            if (find) {
                ...
                const newUser = { id: User.Id, password: User.Password, balance: Math.ceil(Math.random() * 1000) };
...
async function withdrawAmount(id, amount) {
    const find = await findUserId(id);
    if (find) {
        let newUser = users[users.findIndex(x => x.id == id)];
...
async function transferAmount(senderId, reciverId, amount) {
...
async function enableFunctionality(loginUser) {
    let repeat = true;
    while (repeat) {
...
async function login() {
    ...
    while (repeat) {
        ...
        if (validateNum(User.Id) && validateNum(User.Password)) {
            const newUser = { id: User.Id, password: User.Password, balance: 0 };
